...
-q, --query           VCF file with query CSVs
-t, --target          VCF file with target CSVs
...
**Genotype parsing**: if each input VCF file contains multiple samples, only the genotype of first sample is retained.
...
- `matches.vcf`: query CSVs annotated with match information (see below)
- `report.json`: summary statistics
- `report.pdf`: diagnostic plots (only with `--plot`)
- `query.vcf` and `target.vcf`: CSVs assembled from the query and target input VCFs, respectively 
(in a `single` record VCF format, with all breakpoints listed in the `BKPS` field)

<a name="benchvcf"></a>
##### Annotated VCF (`matches.vcf`)

Contains the assembled query CSVs (one line per CSV). Each record includes:
...
```
chr2  1450200  q_42  <skip> END=1492800;SVTYPE=INV+DUP;BKPS=chr2:1450200,chr2:1471050,chr2:1488300,chr2:1492800;
...
A report with various stats, including: 

- `n_query`: number of query CSVs after parsing and size filtering
- `n_target`: number of target CSVs after parsing and size filtering  
...
Contains one record per CSV cluster (group of matching CSVs); the CSV with the most breakpoints 
...
- `SUPPORT`: number of distinct input callsets contributing CSVs to this cluster
- `SUPPORT_COUNT`: comma-separated count vector, one entry per input VCF, giving its number of CSVs contributed to this cluster
- `SUPPORT_BINARY`: concatenated 0/1 string, where 1 indicates that the input VCF has CSV in this cluster, 0 otherwise
